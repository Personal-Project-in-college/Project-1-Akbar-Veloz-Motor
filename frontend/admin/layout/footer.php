<!-- Footer -->
<footer class="footer">
  <div class="container">
    <div class="d-sm-flex justify-content-center justify-content-sm-between">
      <span class="text-muted text-center text-sm-left d-block d-sm-inline-block">
        Copyright © 2025. Akbar Veloz Motor
      </span>
      <span class="float-none float-sm-right d-block mt-1 mt-sm-0 text-center">
        Hand-crafted & made with <i class="ti-heart text-danger ms-1"></i>
      </span>
    </div>
  </div>
</footer>
</div><!-- content-wrapper end -->
</div><!-- page-body-wrapper end -->

<!-- JavaScript -->
<script src="../src/assets/vendors/js/vendor.bundle.base.js"></script>
<script src="../src/assets/vendors/chart.js/chart.umd.js"></script>
<script src="../src/assets/vendors/datatables.net/jquery.dataTables.js"></script>
<script src="../src/assets/vendors/datatables.net-bs5/dataTables.bootstrap5.js"></script>
<script src="../src/assets/js/dataTables.select.min.js"></script>
<script src="../src/assets/js/off-canvas.js"></script>
<script src="../src/assets/js/template.js"></script>
<script src="../src/assets/js/settings.js"></script>
<script src="../src/assets/js/todolist.js"></script>
<script src="../src/assets/js/jquery.cookie.js" type="text/javascript"></script>
<script src="../src/assets/js/dashboard.js"></script>
<script src="../src/assets/js/table_controls.js"></script>
</body>
</html>
