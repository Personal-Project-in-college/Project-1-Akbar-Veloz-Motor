...
$stmt = $koneksi->prepare("SELECT vehicle_models.*, brands.name AS brand_name FROM vehicle_models JOIN brands ON vehicle_models.brand_id = brands.id WHERE vehicle_models.deleted_at IS NULL AND vehicle_models.deleted_by_brand_at IS NULL AND vehicle_models.name LIKE ? ORDER BY brands.name ASC");
